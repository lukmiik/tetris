...
class Rules:
    ...
    def create_draw_text(self) -> None:
        ...
        for idx, line in enumerate(self.TEXT):
            text_rendered = self.settings.font_rules.render(line, True, self.settings.FONT_COLOR)
    ...
    def check_events(self) -> bool | None:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN and self.settings.go_back_btn_rect.collidepoint(event.pos):
                return True
